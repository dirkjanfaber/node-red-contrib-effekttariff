...
const scenarios = {
  ...
  dynamicHeadroom: {
    ...
    analysis: `<strong>What this proves:</strong> The "headroom" is a safety buffer below your target limit - it gives your
      ...
      maximizes your usable power while maintaining safety. Watch how limits change throughout the day as SOC varies.`,
    config: {
      peakCount: 3,
      onePeakPerDay: true,
      peakHoursStart: 7,
    ...
    batterySocGenerator: (hour, date, ctx) => {
      ...
      // Returns { soc, minSoc } as expected by simulation
      if (hour < 8) return { soc: 10, minSoc: 10 } // Low SOC
      if (hour < 16) return { soc: 50, minSoc: 10 } // Medium SOC
      return { soc: 90, minSoc: 10 } // High SOC
    },
    expectations: {
      customChecks: [
        {
