...
const scenarios = {
  ...
  batteryBalancing: {
    ...
    expectations: {
      customChecks: [
        {
          name: 'Balancing mode should activate and charge to target',
          expected: true,
          check: (results) => {
            const balancingCharge = results.chargeRateChanges.find(c => c.reason.includes('balancing: charging'))
            return balancingCharge && balancingCharge.chargeRateW > 0
          }
        },
        {
          name: 'Balancing mode should hold target SOC',
          expected: true,
          check: (results) => {
            const balancingHold = results.chargeRateChanges.find(c => c.reason.includes('balancing: holding'))
            return balancingHold && balancingHold.chargeRateW === 0
          }
        },
        {
          name: 'Balancing mode should complete',
          expected: true,
          check: (results) => {
            const balancingComplete = results.chargeRateChanges.find(c => c.reason.includes('balancing: complete'))
            return balancingComplete !== undefined
          }
        }
      ]
    }
