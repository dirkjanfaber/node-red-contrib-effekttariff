...
const scenarios = {
   ...
   * Tests: Periodic battery balancing feature over multiple weeks
   */
  batteryBalancing: {
    name: 'Battery Balancing',
    description: 'Tests the periodic battery balancing feature over 16 days with balancing occurring once.',
    analysis: `<strong>What this proves:</strong> LiFePO4 and lithium batteries benefit from occasional full charges to
      balance cell voltages - typically every 2 weeks. This 16-day simulation shows peak shaving during normal operation,
      with the battery balancing feature charging to 100% SOC during off-peak night hours (00:00-06:00) when conditions
      are met. The system only triggers balancing when SOC reaches the threshold (>95%) naturally through overnight
      charging. This maintains battery health while still achieving significant peak reduction. Watch how the battery
      cycles daily for peak shaving, with the occasional full charge for cell balancing.`,
    config: {
      peakCount: 3,
      onePeakPerDay: true,
      peakHoursStart: 7,
      ...
      minimumLimitKw: 2,
      batteryEnabled: true,
      batteryCapacityWh: 10000, // 10 kWh battery
      maxChargeRateW: 3000,
      maxDischargeRateW: 5000,
      minSoc: 10,
      socBuffer: 70, // Target 80% SOC normally - leaves room for balancing to push to 100%
      batteryBalancing: {
        enabled: true,
        socThreshold: 95,
        targetSoc: 100,
    ...
    durationDays: 16, // ~2 weeks to show balancing cycle
    powerGenerator: powerPatterns.dailyProfile(1500, 3500), // Realistic daily profile
    initialSoc: 50, // Start at 50% - natural cycling will build up SOC
    expectations: {
      minPeaks: 3,
      learningComplete: true,
      customChecks: [
        {
          name: 'Battery balancing is configured and enabled',
          expected: true,
          check: (results) => {
        ...
        {
          name: 'Battery discharged during peak hours for shaving',
          expected: true,
          check: (results) => {
  ...
  /**
   * Learning phase carryover scenario
   * Tests: Previous month carryover during learning phase
   */
  learningCarryover: {
    name: 'Learning Phase Carryover',
    description: 'Tests the learning phase carryover feature across a month boundary.',
    analysis: `<strong>What this proves:</strong> At the start of each month, the system enters a "learning phase" until
      enough peaks are recorded. By default, it uses the minimum limit during learning, which can allow unnecessarily high
      peaks. With "carryover mode" enabled, the system instead uses a percentage (e.g., 80%) of the previous month's peak
      average. This 40-day simulation crosses from January into February, demonstrating how the system maintains reasonable
      limits at the month boundary instead of starting fresh. Watch the limit changes around February 1st - with carryover,
      the system doesn't drop to the minimum limit but continues with an appropriate target based on January's performance.`,
    config: {
      peakCount: 3,
      onePeakPerDay: true,
      peakHoursStart: 7,
    ...
    startDate: new Date('2024-01-20T00:00:00'), // Start late January
    durationDays: 20, // Run into early February
    powerGenerator: powerPatterns.dailyProfile(1500, 3500),
    expectations: {
      minPeaks: 3,
      learningComplete: true,
      customChecks: [
        {
          name: 'Month reset occurred during simulation',
          expected: true,
          check: (results) => {
            // Check that we crossed a month boundary
            const startMonth = new Date(results.config.startDate || '2024-01-20').getMonth()
            const endDate = new Date(results.hourlyData[results.hourlyData.length - 1]?.date || '2024-02-08')
            const endMonth = endDate.getMonth()
            return startMonth !== endMonth
          }
        },
        {
          name: 'System used carryover after month reset',
          expected: true,
          check: (results) => {
            // After month reset, the system should have used carryover
            // We check that we have recorded peaks in February
            const febPeaks = results.finalState.peaks.filter(p => {
              const month = new Date(p.date).getMonth()
              return month === 1 // February
            })
            return febPeaks.length > 0
          }
        }
      ]
    }
  ...
  // ============================================================================
  // FLANDERS (BELGIUM) SCENARIOS (Capaciteitstarief)
  // ============================================================================

  /**
   * Flanders basic scenario
   * Tests: Flemish capacity tariff with 15-minute intervals and single peak per month
   */
  belgiumBasic: {
    name: 'Flanders Basic',
    description: 'Flemish capacity tariff (capaciteitstarief) with 15-minute intervals and single peak per month',
    analysis: `<strong>What this proves:</strong> The Flemish capacity tariff (capaciteitstarief) uses a different model than Sweden.
    ...
    config: {
      ...
      phases: 3,
      gridVoltage: 230,
      maxBreakerCurrent: 40
    },
    startDate: new Date('2024-01-01T00:00:00'),
    durationDays: 30,
    powerGenerator: powerPatterns.dailyProfile(1500, 4000),
    expectations: {
      // Note: Belgium uses singlePeakMode, so traditional "learningComplete" doesn't apply
      customChecks: [
        {
          name: 'Single peak tracked per month (Belgium-style)',
          expected: true,
          check: (results) => results.finalState.currentMonthPeak !== null
        },
        {
          name: 'Peak measurement is 24/7 (no peak hour restrictions)',
          expected: true,
          check: (results) => {
            return results.config.peakHoursStart === 0 && results.config.peakHoursEnd === 24
          }
        }
      ]
    }
  ...
  /**
   * Flanders EV charging scenario
   * Tests: Impact of occasional 11kW EV charging on Flemish capacity tariff
   */
  belgiumWithEV: {
    name: 'Flanders EV Charging',
    description: 'Flemish household with occasional 11kW EV charging showing peak impact',
    analysis: `<strong>What this proves:</strong> In Flanders, a single 15-minute period of 11kW EV charging creates an 11kW peak
      that affects your capacity fee for the entire year. This simulation shows a typical Flemish household with occasional evening
      EV charging sessions. Without intervention, these EV charging peaks become your monthly capacity, potentially costing an
      extra €300-400/year compared to a lower baseline. With a home battery, these peaks can be shaved by discharging stored energy
      during EV charging. This demonstrates why peak shaving is especially valuable for EV owners in Flanders - limiting charging peaks
      directly reduces your annual capacity fee.`,
    config: {
      region: 'belgium',
      measurementIntervalMinutes: 15,
      singlePeakMode: true,
    ...
    durationDays: 30,
    powerGenerator: (hour, date, ctx) => {
      const base = powerPatterns.dailyProfile(1200, 3000)(hour)
      // 11kW EV charging on certain evenings (days 5, 12, 19, 26)
      if ([5, 12, 19, 26].includes(ctx.dayOfMonth) && hour >= 18 && hour <= 21) {
        return base + 8000 // ~11kW total with base load
      }
      return base
    },
    expectations: {
      ...
      customChecks: [
        {
          name: 'High EV charging peak recorded',
          expected: true,
          check: (results) => {
            const peak = results.finalState.currentMonthPeak
            return peak && peak.effective > 8000
          }
        }
      ]
    }
  ...
  /**
   * Flanders three-phase unbalanced load scenario
   * Tests: threshold-based limiting for capaciteitstarief
   * Demonstrates that with thresholdLimiting enabled, the current limit is only
   * applied when the 15-minute running average actually reaches the peak target —
   * avoiding unnecessary battery discharge on unbalanced three-phase loads.
   */
  belgiumThreephase: {
    name: 'Flanders Three-Phase Unbalanced',
    description: 'Three-phase household where loads are unbalanced across phases, showing threshold-based limiting',
    analysis: `<strong>What this proves:</strong> On a three-phase system the output current limit is applied equally to all phases
      (e.g. 2500 W &divide; 3 &times; 230 V &asymp; 3.6 A per phase). With standard limiting, this value is active whenever a monthly
      peak has been recorded — even when current consumption is well below the threshold on a single phase. A washing machine or
      heat pump drawing 2.2 kW on L1 (9.6 A) would be throttled to 3.6 A, causing unnecessary battery discharge even though
      total consumption is within the allowed peak.<br><br>
      With <strong>Threshold-based limiting</strong> enabled the node waits until the 15-minute running average actually reaches
      the configured peak target before applying a restriction. This means the single-phase load runs freely at 2.2 kW and the
      battery stays untouched. The limit only activates during the occasional spikes (EV charging, heat pump at full power)
      where the running average genuinely threatens to create a new monthly peak.<br><br>
      The chart shows the limit line: with threshold limiting it sits at the maximum breaker value (unrestricted) for most of
      the month, only dropping during real peak events. This is the recommended mode for three-phase capaciteitstarief installations
    ...
    config: {
      ...
      gridVoltage: 230,
      maxBreakerCurrent: 25,
      thresholdLimiting: true
    },
    startDate: new Date('2024-01-01T00:00:00'),
    durationDays: 30,
    powerGenerator: (hour, date, ctx) => {
      // Typical household: base load on one phase (1.0-2.2 kW), occasionally a spike
      const base = powerPatterns.dailyProfile(1000, 2200)(hour)
      // Occasional heavy load events (e.g. EV charging or heat pump full power) on days 8, 16, 24
      if ([8, 16, 24].includes(ctx.dayOfMonth) && hour >= 19 && hour <= 21) {
        return base + 1800 // ~4 kW total — exceeds the 2.5 kW threshold
      }
      return base
    },
    expectations: {
      customChecks: [
        {
          name: 'Monthly peak recorded',
          expected: true,
          check: (results) => {
            return results.finalState.currentMonthPeak !== null
          }
        },
        {
          name: 'Peak is from spike events, not from base load',
          expected: true,
          check: (results) => {
            const peak = results.finalState.currentMonthPeak
            // Base load peaks around 2.2 kW; spike events push above 3 kW
            return peak && peak.effective > 2800
          }
        }
      ]
    }
  ...
  /**
   * Flanders annual rolling average scenario
   * Tests: 12-month rolling average calculation
   */
  belgiumAnnualRolling: {
    name: 'Flanders Annual Rolling',
    description: 'Full year simulation showing 12-month rolling average for Flemish billing',
    analysis: `<strong>What this proves:</strong> Flanders calculates your annual capacity fee based on the average of your monthly
    ...
    startDate: new Date('2024-01-01T00:00:00'),
    durationDays: 395, // ~13 months to show rollover
    powerGenerator: (hour, date, ctx) => {
      // High peaks in January (month 1), then normal for rest of year
      const month = date.getMonth() + 1
      if (month === 1 && ctx.dayOfMonth <= 7) {
        // High consumption in first week of January (EV charging)
        return powerPatterns.dailyProfile(1500, 8000)(hour)
      }
      // Normal consumption rest of the year
      return powerPatterns.dailyProfile(1500, 3500)(hour)
    },
    expectations: {
      customChecks: [
        {
          name: '12+ monthly peaks stored for rolling average',
          expected: true,
          check: (results) => {
            return results.finalState.monthlyPeaks && results.finalState.monthlyPeaks.length >= 12
          }
        },
        {
          name: 'Rolling average calculated from monthly peaks',
          expected: true,
          check: (results) => {
            const monthlyPeaks = results.finalState.monthlyPeaks
            if (!monthlyPeaks || monthlyPeaks.length === 0) return false
            const sum = monthlyPeaks.reduce((acc, m) => acc + m.peakW, 0)
            const avg = sum / monthlyPeaks.length
            return avg > 0
          }
        }
      ]
