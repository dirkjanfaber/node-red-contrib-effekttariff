...
function runSimulation (options) {
  ...
  // Detect Belgium mode
  const isBelgiumMode = config.singlePeakMode === true || config.region === 'belgium'
  ...
  const results = {
    ...
    totalSamples: 0,
    hourlyData: [],
    intervalData: [], // Belgium: 15-minute interval data
    monthResets: [],
    peakRecords: [],
    outputChanges: [],
    chargeRateChanges: [],
    ...
    baselineHourlyData: [],
    baselineIntervalData: [], // Belgium baseline
    baselinePeaks: [],
    // Belgium mode flag
    isBelgiumMode
  }
  ...
  for (let i = 0; i < totalSamples; i++) {
    ...
    // Track hour completions and peak records (Sweden mode)
    ...
    // Track interval completions (Belgium mode)
    if (result.intervalCompleted) {
      const intervalMinutesConfig = config.measurementIntervalMinutes || 60
      const completedIntervalDate = new Date(currentTime.getTime() - intervalMinutesConfig * 60000)
      const intervalDataPoint = {
        date: completedIntervalDate,
        intervalId: result.intervalCompleted.intervalId,
        avgW: result.intervalCompleted.avgW,
        effectiveW: result.intervalCompleted.effectiveW,
        rawAvgW: rawPower,
        batteryContribution: dischargeRateW
      }
      results.intervalData.push(intervalDataPoint)

      // Track baseline interval data
      if (baselineResult.intervalCompleted) {
        results.baselineIntervalData.push({
          date: completedIntervalDate,
          intervalId: baselineResult.intervalCompleted.intervalId,
          avgW: baselineResult.intervalCompleted.avgW,
          effectiveW: baselineResult.intervalCompleted.effectiveW
        })
      }

      // Log Belgium peak updates
      if (result.intervalCompleted.peakUpdated) {
        const batteryNote = dischargeRateW > 0 ? ` [battery: -${Math.round(dischargeRateW)}W]` : ''
        log(`[BELGIUM PEAK] ${currentTime.toISOString().slice(0, 16)} interval ${result.intervalCompleted.intervalId}: ${Math.round(result.intervalCompleted.avgW)}W (eff: ${Math.round(result.intervalCompleted.effectiveW)}W)${batteryNote}`)
      }
    }
...
function generateSummary (results, config) {
  const { hourlyData, intervalData, peakRecords, outputChanges, finalState, baselineFinalState, batteryData, isBelgiumMode } = results

  // Detect Belgium mode
  const belgiumMode = isBelgiumMode || config.singlePeakMode === true || config.region === 'belgium'

  // Calculate statistics based on mode
  let avgPower, maxPower, minPower, dataPoints

  if (belgiumMode && intervalData && intervalData.length > 0) {
    // Belgium mode: use interval data
    const intervalAvgs = intervalData.map(i => i.avgW)
    avgPower = intervalAvgs.reduce((a, b) => a + b, 0) / intervalAvgs.length
    maxPower = Math.max(...intervalAvgs)
    minPower = Math.min(...intervalAvgs)
    dataPoints = intervalData.length
  } else {
    // Sweden mode: use hourly data
    const hourlyAvgs = hourlyData.map(h => h.avgW)
    avgPower = hourlyAvgs.length > 0
      ? hourlyAvgs.reduce((a, b) => a + b, 0) / hourlyAvgs.length
      : 0
    maxPower = hourlyAvgs.length > 0 ? Math.max(...hourlyAvgs) : 0
    minPower = hourlyAvgs.length > 0 ? Math.min(...hourlyAvgs) : 0
    dataPoints = hourlyData.length
  }

  // Get final peaks based on mode
  let peakAverage, finalPeaks, baselinePeakAverage, baselinePeaks

  if (belgiumMode) {
    // Belgium: single peak per month
    const currentMonthPeak = finalState.currentMonthPeak
    peakAverage = currentMonthPeak ? currentMonthPeak.effective : 0
    finalPeaks = currentMonthPeak ? [currentMonthPeak] : []

    // Baseline for Belgium
    const baselineMonthPeak = baselineFinalState ? baselineFinalState.currentMonthPeak : null
    baselinePeakAverage = baselineMonthPeak ? baselineMonthPeak.effective : 0
    baselinePeaks = baselineMonthPeak ? [baselineMonthPeak] : []
  } else {
    // Sweden: average of top N peaks
    finalPeaks = finalState.peaks.slice(0, config.peakCount)
    peakAverage = finalPeaks.length > 0
      ? finalPeaks.reduce((sum, p) => sum + p.effective, 0) / finalPeaks.length
      : 0

    baselinePeaks = baselineFinalState ? baselineFinalState.peaks.slice(0, config.peakCount) : []
    baselinePeakAverage = baselinePeaks.length > 0
  ...
  const analysis = {
    ...
    baselineTopPeaks: baselinePeaks.map(p => ({
      date: p.date || p.intervalId,
      hour: p.hour || (p.time ? parseInt(p.time.split(':')[0]) : 0),
  ...
  // Belgium-specific analysis
  let belgiumAnalysis = null
  if (belgiumMode) {
    const monthlyPeaks = finalState.monthlyPeaks || []
    const rollingMonths = config.rollingMonths || 12
    const annualBillingEnabled = config.annualBillingEnabled

    // Calculate rolling average if annual billing is enabled
    let rollingAverageW = peakAverage
    if (annualBillingEnabled && monthlyPeaks.length > 0) {
      const peaksToAverage = monthlyPeaks.slice(-rollingMonths)
      rollingAverageW = peaksToAverage.reduce((sum, p) => sum + p.peakW, 0) / peaksToAverage.length
    }

    // Baseline rolling average
    const baselineMonthlyPeaks = baselineFinalState ? (baselineFinalState.monthlyPeaks || []) : []
    let baselineRollingAverageW = baselinePeakAverage
    if (annualBillingEnabled && baselineMonthlyPeaks.length > 0) {
      const baselinePeaksToAverage = baselineMonthlyPeaks.slice(-rollingMonths)
      baselineRollingAverageW = baselinePeaksToAverage.reduce((sum, p) => sum + p.peakW, 0) / baselinePeaksToAverage.length
    }

    // Belgium cost: ~€50/kW/year based on rolling average
    const costPerKwYear = 50 // EUR
    const achievedAnnualCostEur = (rollingAverageW / 1000) * costPerKwYear
    const baselineAnnualCostEur = (baselineRollingAverageW / 1000) * costPerKwYear
    const annualSavingsEur = baselineAnnualCostEur - achievedAnnualCostEur

    belgiumAnalysis = {
      measurementInterval: config.measurementIntervalMinutes || 60,
      currentMonthPeak: finalState.currentMonthPeak,
      baselineMonthPeak: baselineFinalState ? baselineFinalState.currentMonthPeak : null,
      monthlyPeaksCount: monthlyPeaks.length,
      rollingMonths,
      annualBillingEnabled,
      rollingAverageW: Math.round(rollingAverageW),
      rollingAverageKw: Math.round(rollingAverageW / 100) / 10,
      baselineRollingAverageW: Math.round(baselineRollingAverageW),
      baselineRollingAverageKw: Math.round(baselineRollingAverageW / 100) / 10,
      // Cost estimates in EUR
      achievedAnnualCostEur: Math.round(achievedAnnualCostEur),
      baselineAnnualCostEur: Math.round(baselineAnnualCostEur),
      annualSavingsEur: Math.round(annualSavingsEur),
      monthlySavingsEur: Math.round(annualSavingsEur / 12)
    }

    // Update analysis with Belgium costs
    analysis.estimatedAnnualSavingsEur = belgiumAnalysis.annualSavingsEur
    analysis.estimatedMonthlySavingsEur = belgiumAnalysis.monthlySavingsEur
  }

  // Format peak data consistently
  const formatPeak = (p) => {
    if (belgiumMode && p) {
      return {
        date: p.date || p.intervalId,
        hour: p.time ? parseInt(p.time.split(':')[0]) : 0,
        time: p.time || '00:00',
        valueW: Math.round(p.value),
        effectiveW: Math.round(p.effective)
      }
  ...
  return {
    belgiumMode,
    hourlyStats: {
      totalHours: dataPoints,
      avgPowerW: Math.round(avgPower),
      maxPowerW: Math.round(maxPower),
      minPowerW: Math.round(minPower),
      label: belgiumMode ? `${config.measurementIntervalMinutes || 60}-min intervals` : 'hours'
    },
    peakStats: {
      totalRecorded: belgiumMode ? (finalState.currentMonthPeak ? 1 : 0) : finalState.peaks.length,
      added: peakAdded,
      updated: peakUpdated,
      finalTopPeaks: finalPeaks.map(formatPeak),
...
function formatResults (results) {
  ...
  const lines = []
  const isBelgium = summary.belgiumMode

  lines.push('╔════════════════════════════════════════════════════════════════╗')
  lines.push(`║              ${isBelgium ? 'CAPACITEITSTARIEF' : 'EFFEKTTARIFF'} SIMULATION RESULTS${isBelgium ? '          ' : '                   '}║`)
  lines.push('╠════════════════════════════════════════════════════════════════╣')
  ...
  if (isBelgium) {
    lines.push(`║ Region: Flanders, Belgium (${config.measurementIntervalMinutes || 60}-minute intervals)`)
  }
  lines.push('╠════════════════════════════════════════════════════════════════╣')

  lines.push('║ CONFIGURATION:')
  if (isBelgium) {
    lines.push(`║   Measurement interval: ${config.measurementIntervalMinutes || 60} minutes`)
    lines.push(`║   Peak tracking: Single highest per month`)
    lines.push(`║   Annual billing: ${config.annualBillingEnabled ? `Yes (${config.rollingMonths}-month rolling)` : 'No'}`)
  } else {
  ...
  const statsLabel = isBelgium ? 'INTERVAL STATISTICS' : 'HOURLY STATISTICS'
  lines.push(`║ ${statsLabel}:`)
  lines.push(`║   Total ${summary.hourlyStats.label} recorded: ${summary.hourlyStats.totalHours}`)
  ...
  if (isBelgium) {
    const peak = summary.belgiumAnalysis?.currentMonthPeak
    if (peak) {
      lines.push(`║   Current month peak: ${(peak.effective / 1000).toFixed(2)} kW`)
      lines.push(`║   Peak recorded: ${peak.date} ${peak.time}`)
    } else {
      lines.push('║   Current month peak: (none recorded)')
    }
    if (summary.belgiumAnalysis?.annualBillingEnabled) {
      lines.push(`║   Monthly peaks stored: ${summary.belgiumAnalysis.monthlyPeaksCount}`)
      lines.push(`║   Rolling average (${summary.belgiumAnalysis.rollingMonths}mo): ${summary.belgiumAnalysis.rollingAverageKw} kW`)
    }
  } else {
  ...
  if (isBelgium && summary.belgiumAnalysis) {
    lines.push('║ COST ANALYSIS (Flanders, Belgium ~€50/kW/year):')
    if (config.batteryEnabled && summary.belgiumAnalysis.baselineMonthPeak) {
      lines.push(`║   Without battery: ${summary.belgiumAnalysis.baselineRollingAverageKw} kW → €${summary.belgiumAnalysis.baselineAnnualCostEur}/year`)
      lines.push(`║   With battery: ${summary.belgiumAnalysis.rollingAverageKw} kW → €${summary.belgiumAnalysis.achievedAnnualCostEur}/year`)
      lines.push(`║   Annual savings: €${summary.belgiumAnalysis.annualSavingsEur}/year (€${summary.belgiumAnalysis.monthlySavingsEur}/month)`)
    } else {
      lines.push(`║   Current peak: ${summary.belgiumAnalysis.rollingAverageKw} kW`)
      lines.push(`║   Estimated annual cost: €${summary.belgiumAnalysis.achievedAnnualCostEur}/year`)
    }
    lines.push('╠════════════════════════════════════════════════════════════════╣')
  }
...
function generateHTMLReport (results, outputPath, options = {}) {
  ...
  const isBelgiumReport = results.summary.belgiumMode
...
<head>
  ...
  <title>${isBelgiumReport ? 'Capaciteitstarief' : 'Effekttariff'} Simulation Report</title>
...
<body>
  <div class="container">
    <h1>${isBelgiumReport ? 'Capaciteitstarief' : 'Effekttariff'} Simulation Report</h1>
    ...
    <div class="meta">
      <div class="meta-grid">
        ...
        ${isBelgiumReport ? `
        <div class="meta-item">
          <div class="meta-label">Measurement</div>
          <div class="meta-value">${results.config.measurementIntervalMinutes || 60}-min intervals</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Peak Tracking</div>
          <div class="meta-value">Single peak/month</div>
        </div>
        ` : `
        <div class="meta-item">
        ...
        <div class="meta-item">
          <div class="meta-label">${isBelgiumReport ? 'Month Peak' : 'Peak Average'}</div>
...
: ''}
    ...
    <div class="analysis">
      ...
      ${results.config.batteryEnabled ? (isBelgiumReport ? `
      <div class="analysis-grid">
        <div class="analysis-item">
          <div class="analysis-label">Without Battery</div>
          <div class="analysis-value">${results.summary.belgiumAnalysis?.baselineRollingAverageKw || results.summary.analysis.baselinePeakAverageKw} kW</div>
          <div class="analysis-subtext">Unmanaged monthly peak</div>
        </div>
        <div class="analysis-item">
          <div class="analysis-label">With Battery</div>
          <div class="analysis-value">${results.summary.belgiumAnalysis?.rollingAverageKw || results.summary.analysis.achievedPeakAverageKw} kW</div>
          <div class="analysis-subtext">Battery-shaved peak</div>
        </div>
        <div class="analysis-item">
      ...
      <div class="analysis-explanation">
        <strong>Annual Cost Impact:</strong> The Flemish capacity tariff costs approximately €50/kW/year based on your 12-month rolling average.
        This ${results.summary.analysis.reductionKw} kW reduction saves approximately
        <strong>€${results.summary.belgiumAnalysis?.annualSavingsEur || 0}/year</strong> (€${results.summary.belgiumAnalysis?.monthlySavingsEur || 0}/month).
        ${results.summary.batteryAnalysis ? `
        <br/><br/><strong>Battery Contribution:</strong> The battery discharged during ${results.summary.batteryAnalysis.totalDischargeHours} intervals
        ...
        ` : ''}
        <br/><br/><strong>Why this matters in Flanders:</strong> A single 15-minute high-power event (like 11kW EV charging) sets your monthly peak,
        which then affects your annual fee for 12 months. Peak shaving is especially valuable here.
      </div>
      <table class="comparison-table">
        ...
        <tbody>
          <tr>
            <td>Monthly Peak</td>
            <td>${results.summary.belgiumAnalysis?.baselineRollingAverageKw || results.summary.analysis.baselinePeakAverageKw} kW</td>
            <td>${results.summary.belgiumAnalysis?.rollingAverageKw || results.summary.analysis.achievedPeakAverageKw} kW</td>
          ...
          <tr>
            ...
            <td>€${results.summary.belgiumAnalysis?.baselineAnnualCostEur || Math.round(results.summary.analysis.baselinePeakAverageKw * 50)}</td>
            <td>€${results.summary.belgiumAnalysis?.achievedAnnualCostEur || Math.round(results.summary.analysis.achievedPeakAverageKw * 50)}</td>
            <td class="reduction">${results.summary.belgiumAnalysis?.annualSavingsEur > 0 ? '-€' + results.summary.belgiumAnalysis.annualSavingsEur : '-'}</td>
          </tr>
          <tr>
            <td>Est. Monthly Cost*</td>
            <td>€${Math.round((results.summary.belgiumAnalysis?.baselineAnnualCostEur || 0) / 12)}</td>
            <td>€${Math.round((results.summary.belgiumAnalysis?.achievedAnnualCostEur || 0) / 12)}</td>
            <td class="reduction">${results.summary.belgiumAnalysis?.monthlySavingsEur > 0 ? '-€' + results.summary.belgiumAnalysis.monthlySavingsEur : '-'}</td>
          </tr>
        </tbody>
      </table>
      ` : `
      ...
      <div class="analysis-grid">
        <div class="analysis-item">
          <div class="analysis-label">Current Month Peak</div>
          <div class="analysis-value">${results.summary.belgiumAnalysis?.rollingAverageKw || results.summary.peakStats.peakAverageKw} kW</div>
          <div class="analysis-subtext">Highest 15-min interval</div>
        </div>
        <div class="analysis-item">
          <div class="analysis-label">Est. Annual Cost*</div>
          <div class="analysis-value">€${results.summary.belgiumAnalysis?.achievedAnnualCostEur || Math.round(results.summary.peakStats.peakAverageKw * 50)}</div>
          <div class="analysis-subtext">Capacity tariff portion</div>
        </div>
        <div class="analysis-item">
          <div class="analysis-label">Est. Monthly Cost*</div>
          <div class="analysis-value">€${Math.round((results.summary.belgiumAnalysis?.achievedAnnualCostEur || 0) / 12)}</div>
          <div class="analysis-subtext">Per month average</div>
        </div>
      </div>
      <div class="analysis-explanation">
        <strong>Why No Savings?</strong> This simulation has no battery configured. The capaciteitstarief node outputs a
        ...
        <br/><br/>
        <strong>What This Shows:</strong> The system correctly tracks your highest 15-minute peak during the month.
        Your current peak of <strong>${results.summary.belgiumAnalysis?.rollingAverageKw || results.summary.peakStats.peakAverageKw} kW</strong>
        results in an estimated annual capacity fee of <strong>€${results.summary.belgiumAnalysis?.achievedAnnualCostEur || 0}</strong>.
        <br/><br/>
        ...
        <strong>€${Math.round((11 - 5) * 50)}/year</strong> on your capacity tariff.
      </div>
      <table class="comparison-table">
        <thead>
          <tr>
            <th>Current Month Peak</th>
            <th>Date/Time</th>
            <th>Power (kW)</th>
          </tr>
        </thead>
        <tbody>
          ${results.summary.belgiumAnalysis?.currentMonthPeak ? `
          <tr>
            <td>Highest Peak</td>
            <td>${results.summary.belgiumAnalysis.currentMonthPeak.date} ${results.summary.belgiumAnalysis.currentMonthPeak.time}</td>
            <td>${(results.summary.belgiumAnalysis.currentMonthPeak.effective / 1000).toFixed(2)}</td>
          </tr>
          ` : `
          <tr>
            <td colspan="3">No peaks recorded yet</td>
          </tr>
          `}
        </tbody>
      </table>
      ...
      `)}
      <div style="font-size: 11px; opacity: 0.7; margin-top: 10px;">*Estimated using ${isBelgiumReport ? 'Flemish capacity tariff rate of ~€50/kW/year' : 'typical Swedish effekttariff rate of ~50 SEK/kW/month'}. Actual rates vary by provider.</div>
    </div>

    <div class="charts-grid">
      <div class="chart-container">
        <h3 class="chart-title">${isBelgiumReport ? 'Interval' : 'Hourly'} Power Consumption vs Limit</h3>
        <canvas id="powerChart"></canvas>
      </div>

      <div class="chart-container">
        <h3 class="chart-title">${isBelgiumReport ? 'Monthly Peak' : `Top ${results.config.peakCount} Recorded Peaks`}</h3>
