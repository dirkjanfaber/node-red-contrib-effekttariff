 ...
 * @param {function} [options.batteryGenerator] - Optional function (hour, date) => { soc, minSoc }
...
function runSimulation (options) {
  ...
  for (let i = 0; i < totalSamples; i++) {
    ...
    // Generate power for this sample
    const power = powerGenerator(hour, currentTime, { dayOfWeek, dayOfMonth, month })

    // Process through peak tracker
    const result = peakTracker.processGridPower(state, config, power, currentTime)
    ...
    if (result.hourCompleted) {
      ...
      if (result.hourCompleted.result !== 'kept') {
        ...
        log(`[PEAK ${result.hourCompleted.result.toUpperCase()}] ${currentTime.toISOString().slice(0, 10)} hour ${result.hourCompleted.hour}: ${Math.round(result.hourCompleted.avgW)}W (eff: ${Math.round(result.hourCompleted.effectiveW)}W)`)
      }
    }
    ...
    // Process battery state if generator provided
    if (batteryGenerator && config.batteryEnabled) {
      const batteryState = batteryGenerator(hour, currentTime, { dayOfWeek, dayOfMonth, month })
      const chargeResult = peakTracker.calculateChargeRate(config, batteryState, currentTime)

      // Track hourly battery data
      if (hour !== lastHour) {
        results.batteryData.push({
          ...
          chargeRateW: chargeResult.chargeRateW,
          charging: chargeResult.charging,
          reason: chargeResult.reason
        })
      }

      // Track charge rate changes
      const lastChargeRate = results.chargeRateChanges.length > 0
        ...
        : null
      if (chargeResult.chargeRateW !== lastChargeRate) {
        results.chargeRateChanges.push({
          ...
          reason: chargeResult.reason,
          soc: batteryState.soc,
          targetSoc: chargeResult.targetSoc
        })
        log(`[CHARGE RATE] ${currentTime.toISOString().slice(0, 16)} -> ${chargeResult.chargeRateW}W (${chargeResult.reason})`)
      }
    }
    ...
    if (verbose && hour !== lastHour) {
      ...
      log(`[${currentTime.toISOString().slice(0, 13)}:00] ${status} | Limit: ${result.outputLimitA}A | Peaks: ${result.topPeaks.length}`)
      lastHour = hour
    }
...
function generateHTMLReport (results, outputPath, options = {}) {
  ...
  // Build limit data aligned with hourly data
  const limitData = []
  let currentLimit = null
  results.hourlyData.forEach(h => {
    const change = results.outputChanges.find(c =>
      c.date <= h.date && (!currentLimit || c.date > currentLimit.date)
    )
    if (change) currentLimit = change
    const limitW = currentLimit
      ? currentLimit.newLimitA * results.config.phases * results.config.gridVoltage
      : null
    limitData.push(limitW ? Math.round(limitW) : null)
  })
