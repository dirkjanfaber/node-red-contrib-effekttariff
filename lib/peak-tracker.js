...
const DEFAULT_CONFIG = {
  ...
  // Threshold-based limiting: only apply current limit when the running average
  // of the current measurement interval actually reaches the peak threshold.
  // When false (default): limit is always active once a peak has been recorded.
  // When true: no restriction until currentIntervalAvg >= targetLimitW.
  // Useful for three-phase systems with unbalanced loads (capaciteitstarief).
  thresholdLimiting: false,
...
function migrateState (state) {
  ...
  // Add carryover field if missing (added in v0.1.3)
  if (state.previousMonthPeakAvgW === undefined) state.previousMonthPeakAvgW = null
...
function processGridPower (state, config, gridPowerW, now, batteryState) {
  ...
  } else if (config.thresholdLimiting && targetLimitW !== null && currentAvgW < targetLimitW) {
    // Threshold-based: only restrict when the running interval average reaches the target.
    // Helps three-phase systems with unbalanced loads (capaciteitstarief): prevents
    // unnecessary battery discharge when a single phase carries load below the total peak.
    outputLimitA = config.maxBreakerCurrent
  } else {
    // Active limiting
 ...
 * @param {string} stateFile - Absolute path to state file
 * @returns {object|null}
 */
...
/**
 * Atomically persist state to a JSON file (write to .tmp then rename).
