...
function processGridPower (state, config, gridPowerW, now, batteryState) {
  ...
  const month = now.getUTCMonth() + 1 // Months are 0-indexed in JS, 1-indexed in config
  ...
  const result = {
    ...
    hourCompleted: null,
    peakResult: null,
    downtime: null // Add downtime property to result
  }
  ...
  if (state.currentHour !== null && state.currentHour !== currentHour) {
    ...
    if (enabled && action !== 'ignore') {
      const hourDiff = (currentHour - state.currentHour + 24) % 24 // Handle midnight wrap
      if (hourDiff >= triggerHours) {
        result.downtime = {
          fromHour: state.currentHour,
          toHour: currentHour,
          missedHours: hourDiff - 1 // Number of full hours missed
        }
      }
    }
    ...
    if (state.currentHourSamples > 0) {
      ...
      // This is local to the recorded hour, so use local time for it
      const completedHourDate = (state.currentHour > currentHour || now.getUTCHours() < state.currentHour)
  ...
  // Calculate limits
  const { targetLimitW, limitReason, isLearning } = calculateTargetLimit(state, config, batteryState)
